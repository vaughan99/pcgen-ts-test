<script lang="ts">
  ...
  // import CampaignSelector from '$lib/components/campaign-selector/CampaignSelector.svelte';
  // import DiceBag from '$lib/components/dice-bag/DiceBag.svelte';

  // Enable vite to lazy load apps as needed.
  const CampaignSelectorGenerator = () => import('$lib/components/campaign-selector/CampaignSelector.svelte');
  const DiceBagGenerator = () => import('$lib/components/dice-bag/DiceBag.svelte');

  const navState = $state<PageNav>('home');
  const activeCampaign = $state<Campaign | null>(null);

  // If the activeCampaign or navState changes, set title.
  $effect(() => {
    const campaignPart = activeCampaign ? activeCampaign.name : 'Select Campaign';
...
<main>
    ...
    {:else if (navState === 'home')}
        ...
        <p>Campaign Path: {activeCampaign.path} <Button>Open</Button></p>
